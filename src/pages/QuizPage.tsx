...
import {
  ...
  getDeviceType,
} from "../components/quiz/helpers/deviceUtils.ts";
...
const QuizPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setReoccurrences((prevReoccurrences) => {
        ...
        const cleanedReoccurrences = prevReoccurrences.filter((r) =>
        ...
        if (cleanedReoccurrences.length !== prevReoccurrences.length) {
          console.log(
            ...
            prevReoccurrences.length - cleanedReoccurrences.length,
            "removed",
          );
        }
  ...
  const handlePeerClose = (conn: DataConnection) => {
    ...
    setPeerConnections((prev) =>
      prev.filter((c) => c.open && c.peer !== conn.peer),
    );
