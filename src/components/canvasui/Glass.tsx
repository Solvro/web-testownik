...
export interface GlassElements {
  ...
  /**
   * Zero-size probes at the output's top-left, top-right, bottom-right and
   * bottom-left corners. They let pointer coordinates be projected correctly
   * when the glass is inside a perspective-transformed CSS3D surface.
   */
  corners?: readonly [HTMLElement, HTMLElement, HTMLElement, HTMLElement];
  /** Called after the first captured HTML texture has reached WebGL. */
  onContentReady?: () => void;
}
...
export function createGlass(
  ...
  const { source, content, output, corners, onContentReady } = elements;
  ...
  function mapViewportPoint(
    ...
    if (corners) {
      ...
      if (
        ...
        const perspectiveDeterminant = dx1 * dy2 - dx2 * dy1;
        ...
        if (Math.abs(perspectiveDeterminant) < 1e-7) {
          a = p1.x - p0.x;
          b = p3.x - p0.x;
          d = p1.y - p0.y;
          e = p3.y - p0.y;
        } else {
          g = (dx3 * dy2 - dx2 * dy3) / perspectiveDeterminant;
          h = (dx1 * dy3 - dx3 * dy1) / perspectiveDeterminant;
          a = p1.x - p0.x + g * p1.x;
          b = p3.x - p0.x + h * p3.x;
          d = p1.y - p0.y + g * p1.y;
          e = p3.y - p0.y + h * p3.y;
        ...
        const matrixA = a - clientX * g;
        const matrixB = b - clientX * h;
        const matrixC = clientX - c;
        const matrixD = d - clientY * g;
        const matrixE = e - clientY * h;
        const matrixF = clientY - f;
        const determinant = matrixA * matrixE - matrixB * matrixD;

        if (Math.abs(determinant) > 1e-7) {
          const u = (matrixC * matrixE - matrixB * matrixF) / determinant;
          const v = (matrixA * matrixF - matrixC * matrixD) / determinant;
          if (Number.isFinite(u) && Number.isFinite(v)) {
            return [
              Math.min(Math.max(u, 0), 1) * output.clientWidth,
              Math.min(Math.max(v, 0), 1) * output.clientHeight,
            ];
          }
        }
      }
    ...
    return [
      ((clientX - rect.left) / Math.max(rect.width, 1)) * output.clientWidth,
      ((clientY - rect.top) / Math.max(rect.height, 1)) * output.clientHeight,
    ];
  }
  ...
  window.addEventListener("landing-hero-progress", reprojectPointer);
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener("landing-hero-progress", reprojectPointer);
    ...
    <div className={className} style={{ position: "relative", ...style }}>
      ...
      <canvas
        ...
        style={{
          ...
          height: "100%",
          pointerEvents: "none",
          visibility: !native || contentReady ? "visible" : "hidden",
        }}
      />
    </div>
  );
