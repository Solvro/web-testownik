...
float map (vec3 p) {
  float radius = uBaseRadius * uTrailLength;
  float d = 1e5;
...
export function createBubble(
  ...
  const { source, content, output, onContentReady } = elements;
  ...
  let contentMaxX = 1;
  let pointerBounds = output.getBoundingClientRect();
  ...
  let time = 0;
  let renderedSize = config.size;
  let renderedTrail = config.trail;

  function activeCount(): number {
    return Math.min(Math.max(Math.ceil(renderedTrail), 1), MAX_TRAIL);
  }
  ...
  function render() {
    ...
    const trailLength = Math.min(Math.max(renderedTrail, 1), MAX_TRAIL);
    const minRes = Math.min(output.width, output.height);
    const headRadius = Math.max(renderedSize, 4) * dpr * presence;
    const baseRadius = (headRadius * 2) / (minRes * trailLength);
    ...
    const pad =
      headRadius +
      ((Math.log(trailLength + 1) / blend) * minRes) / 2 +
    ...
    gl!.uniform1f(uniforms.uTrailLength, trailLength);
  ...
  function frame(now: number) {
    ...
    // Radius and trail length share one damped morph. Keeping these values
    // inside the renderer avoids recreating the WebGL instance, while the
    // fractional final trail sphere fades in/out instead of popping whenever
    // the segment count crosses an integer.
    const kMorph = reducedMotion ? 1 : 1 - Math.exp(-delta * 11);

    headX += (targetX - headX) * kHead;
    headY += (targetY - headY) * kHead;
    renderedSize += (config.size - renderedSize) * kMorph;
    renderedTrail += (config.trail - renderedTrail) * kMorph;
  ...
  function onPointerLeave(event?: PointerEvent) {
    // pointerout bubbles, so moving onto any button or link fires it for the
    // element being left and it reaches the window listener. Only a null
    // relatedTarget means the pointer genuinely left the document; anything
    // else is just a hop between elements and must not dismiss the droplet.
    if (event != null && event.relatedTarget !== null) return;
    presenceTarget = 0;
    hasPointer = false;
    start();
  }
  ...
  function onPointerEnter() {
    pointerBounds = output.getBoundingClientRect();
  }
  ...
  /*
   * Tracked on the window rather than on the captured subtree. The hero renders
   * the device mock-ups in a sibling layer above this one, so listening on the
   * content element meant the bubble froze the moment the pointer crossed the
   * laptop or a mock-up. pointerBounds is viewport-relative, so the maths is
   * unchanged.
   */
  window.addEventListener("pointerenter", onPointerEnter, { passive: true });
  window.addEventListener("pointermove", onPointerMove, { passive: true });
  window.addEventListener("pointerout", onPointerLeave, { passive: true });
  window.addEventListener("scroll", reprojectPointer, { passive: true });
  window.addEventListener("resize", reprojectPointer, { passive: true });
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener("scroll", reprojectPointer);
      window.removeEventListener("resize", reprojectPointer);
...
export interface BubbleProps extends BubbleOptions {
  ...
  className?: string;
  /**
   * Use Chrome's experimental html-in-canvas capture path. The WebGL bubble
   * still renders when this is false, but the page remains ordinary live DOM.
   * This is useful for above-the-fold content where swapping the entire
   * subtree after hydration would create a visible blank frame.
   */
  captureHtml?: boolean;
  enabled?: boolean;
...
export function Bubble({
  ...
  const native = captureHtml && supported && !failed;
    ...
    <div className={className} style={{ position: "relative", ...style }}>
      ...
      {!native || !contentReady ? (
        <div
          ...
          style={{
            position: native ? "absolute" : "relative",
            inset: native ? 0 : undefined,
            zIndex: native ? 1 : undefined,
            width: "100%",
            height: "100%",
            overflow: "hidden",
          }}
      ...
      <canvas
        ...
        style={{
          ...
          pointerEvents: "none",
          visibility:
            enabled && (!native || contentReady) ? "visible" : "hidden",
        }}
      />
    </div>
  );
