...
const ALLOWED_QUIZ_KEYS = [
    ...
    "is_anonymous", "version", "questions", "maintainer"
];
...
export const validateQuiz = (quiz: Quiz): string | null => {
    ...
    if (!containsOnlyAllowedKeys(quiz, ALLOWED_QUIZ_KEYS)) {
        const invalidKeys = Object.keys(quiz).filter((key) => !ALLOWED_QUIZ_KEYS.includes(key));
    ...
    for (const [questionIndex, question] of quiz.questions.entries()) {
        ...
        if (!containsOnlyAllowedKeys(question, ALLOWED_QUESTION_KEYS)) {
            const invalidKeys = Object.keys(question).filter((key) => !ALLOWED_QUESTION_KEYS.includes(key));
            return `Pytanie nr ${questionIndex + 1} zawiera nieprawidłowe właściwości: ${invalidKeys.join(", ")}`;
        }
        ...
        for (const [answerIndex, answer] of question.answers.entries()) {
            ...
            if (!containsOnlyAllowedKeys(answer, ALLOWED_ANSWER_KEYS)) {
                const invalidKeys = Object.keys(answer).filter((key) => !ALLOWED_ANSWER_KEYS.includes(key));
                return `Odpowiedź nr ${answerIndex + 1} w pytaniu nr ${questionIndex + 1} zawiera nieprawidłowe właściwości: ${invalidKeys.join(", ")}`;
            }
