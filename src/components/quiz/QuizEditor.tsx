...
import { validateQuiz } from "./helpers/quizValidation";
...
const QuizEditor: React.FC<QuizEditorProps> = ({
  ...
  const [questions, setQuestions] = useState<Question[]>(
    initialQuiz?.questions && initialQuiz.questions.length
  ...
  const [prevQuestionId, setPrevQuestionId] = useState<number>(() =>
    questions.reduce((max, q) => (q.id > max ? q.id : max), 0),
  );
  ...
  const addQuestion = () => {
    ...
    setPrevQuestionId(newId);
    // Scroll after render
    requestAnimationFrame(() => {
      const el = document.getElementById(`question-${newId}`);
      ...
      if (el) {
        ...
        setTimeout(() => {
          const el2 = document.getElementById(`question-${newId}`);
          if (el2) {
            el2.scrollIntoView({ behavior: "smooth" });
            const textarea2 = el2.querySelector(
              "textarea",
            ) as HTMLTextAreaElement | null;
            if (textarea2) textarea2.focus({ preventScroll: true });
          }
        }, 50);
      }
    });
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="bg-muted/40 flex items-center justify-between gap-3 rounded-md border px-3 py-2">
            ...
            <Switch
              ...
              onCheckedChange={(val) => setAdvancedMode(val)}
            />
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="pointer-events-none sticky bottom-4 z-10 mt-6 flex justify-center sm:bottom-10">
          <div className="bg-background/60 pointer-events-auto -mx-16 flex flex-wrap items-center justify-center gap-3 rounded-md px-6 py-3 shadow-sm backdrop-blur sm:mx-0">
            {onSaveAndClose && (
              <Button disabled={saving} onClick={() => triggerSave(true)}>
                Zapisz i zakończ
              </Button>
            )}
            <Button
